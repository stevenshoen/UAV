...
system = ModSystem(t0=0, full_state=trimmed_state)
...
"""
from full state determine forces and moments from aero forces, prop and gravity

net forces and torques felt by the aircraft
...
x0 = system.state_vector
print('x0:', x0)
t_ini = system.time

t_span = (t_ini, t_ini + dt)
method = system._method

# TODO: prepare to use jacobian in case it is defined
"""
needs the sim.system to call sim.update
"""

sol = solve_ivp(sim.system.fun_wrapped, t_span, x0, method=method,
                **system._options)
...
system._time = sol.t[-1]
system._state_vector = sol.y[:, -1]

new_x = system._state_vector
print('new_x:', new_x)
