...
from systems.dynamic_system import AircraftDynamicSystem
...
class EulerFlatEarthGround(AircraftDynamicSystem):
    ...
    def check_for_ground(self, state, state_dot):
...
#        print('state_dot :', state_dot)
    ...
    def _update_full_system_state_from_state(self, state, state_dot):
        ...
        if below ground level:
            change to ground
            and zero out downward velocity and accleration
        ...
        """
#        state, state_dot = self.check_for_ground(state, state_dot)
