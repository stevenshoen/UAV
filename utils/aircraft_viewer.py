...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct 19 14:40:39 2020
...
class ProjectionViewer:
    ...
    def run_with(self, state):
        ...
        while running:
            ...
            self.clock.tick(loopRate)
#            self.wireframe.quatRotate(angularVeloctiy, 1/loopRate)
            self.display(state.yaw, state.pitch, state.roll)
            pygame.display.flip()

#    def display(self):
    def vec_display(self, vec):
...
#                            (65, 105, 225))
        linewidth = 0.5
        pts = vec
        pygame.draw.line(self.screen, color, False, pts, linewidth)
#        pygame.draw.polygon(self.screen, face.color, pointList)
        
        
    def display(self, yaw, pitch, roll):
        """ Draw the wireframes on the screen. """
        self.screen.fill(self.background)
